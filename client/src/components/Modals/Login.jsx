...
const Login = React.memo(({ display, setDisplay, setOpenSignUp, setOpenForgotPassword }) => {
    const [email, setEmail] = useState('');
    const [password, setPassword] = useState('');
    const [showPassword, setShowPassword] = useState(false);
    const [isPending, setIsPending] = useState(false);

    const url = useMemo(() => `${baseURL}/users/login`, []);
    ...
    const handleForgotPassword = useCallback((e) => {
        ...
        setDisplay(false);
        toast.info('Enter your email address for further assistance.', { autoClose: 6000 });
    }, [setOpenForgotPassword, setDisplay]);
    ...
    const handleOpenSignUp = useCallback(() => {
        setOpenSignUp(true);
        setDisplay(false);
    }, [setOpenSignUp, setDisplay]);

    if (!display) return null;
    return (
        <>
            <div className="fixed inset-0 bg-black bg-opacity-50 transition-opacity"></div>
            <div className="fixed modal-content inset-0 z-20 overflow-y-auto">
                        ...
                        <div className="bg-gradient-to-br from-gray-50 to-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <div className="sm:flex sm:items-start">
                                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                    ...
                                    <img src={logo} alt="" className="mx-auto w-16 hover:animate-pulse" />
                                    <h3 className="text-3xl font-semibold leading-6 text-gray-900 mt-8 mb-4" id="modal-title">
                                        Welcome Back
                                    </h3>
                                    <div className="mt-2">
