...
async function handleCreateApplication(userId, projectId, url) {
  ...
  try {
    ...
    await executeTasks(appName, userId, projectId, refinedList);
  } catch (error) {
...
async function handleCreateApplicationError(
  ...
  try {
    ...
    const selectedProject = await UserModel.getUserProject(userId, projectId);
    let { appName } = selectedProject;
    await executeTasks(appName, userId, projectId, parsedArray);
    ...
    console.error("Error handling create application error:", formattingError);
  }
}

// Function to execute tasks
async function executeTasks(appName, userId, projectId, taskList) {
...
async function handleModifyApplicationError(
  ...
  try {
    ...
    console.error("Error handling modify application error:", formattingError);
  }
}
